...
function postProcessAncienTemperature(decoded,data){
    ...
    if(decoded.temperature_TMXD != undefined){
        decoded.temperature_TMXD = {
            value :  ((decoded.temperature_TMXD[0]<<8 |decoded.temperature_TMXD[1])<<16>>16)*0.0625,
            unit : "°C"
        };
    }
}
...
function frame_type_ancien(encoded){
    ...
    if(oct1 === 0x01){
        ...
        frameType = "Trame de relevé périodique de la température";
    }else if(oct1 === 0x2A){
        frameType = "Dépassement de seuil bas détecté";
    }else if(oct1 === 0x2C){
        frameType = "Dépassement de seuil haut détecté";
    }else{
             frameType = "Type de trame inconnu";
     }
    return frameType;
    ...
    switch (frame_type) {
        ...
        break;
         ...
         case "Trame de fin d'alerte HAUT d'une valeur 4-20mA":
            res_str = res_str + 'ubyte logic_level_dinda;' + 'ubyte[2] current;' + 'ubyte offset;';
         break;
        case "Trame de relevé périodique de la température":
            res_str = res_str + 'ubyte[2] temperature_TMXD;';
            break;
        case "Dépassement de seuil bas détecté":
            res_str = res_str + 'ubyte[2] temperature_TMXD;';
            break;
        case "Dépassement de seuil haut détecté":
            res_str = res_str + 'ubyte[2] temperature_TMXD;';
            break;
        }
    data.data = res_str;
