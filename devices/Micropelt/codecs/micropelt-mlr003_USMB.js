...
function encode_port_1(input) {
  let mode = input.data.userMode ?? "Ambient_Temperature"; // "Ambient_Temperature" or "Valve_Position"
  let safetyMode = input.data.safetyMode ?? "Ambient_Temperature"; // "Ambient_Temperature" or "Valve_Position"
  let setValue = input.data.setValue ?? 19;	// 0-40 for Ambient_Temperature, 0-100 for Valve_Position
  let roomTemperature = input.data.roomTemperature ?? 0; // 0-40
  let safetyValue = input.data.safetyValue ?? 19;  // 0-40 for Ambient_Temperature, 0-100 for Valve_Position
  let radioInterval = input.data.radioInterval ?? 10;  // 5, 10, 60, 120, 480
  let doReferenceRunNow = input.data.doReferenceRunNow ?? 0; // 0 or 1
